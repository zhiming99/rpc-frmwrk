...
CFuseSvcDir* GetSvcDir( const char* path )
{
    auto pIf = GetRootIf();
    ...
    if( pSvr != nullptr )
    {
        ret = pSvr->GetSvcDir( path, pSvcDir );
    }
    else
    {
        ret = pCli->GetSvcDir( path, pSvcDir );
    }
    if( ERROR( ret ) )
        return nullptr;
    return pSvcDir;
...
gint32 CFuseStmFile::fs_unlink( const char* path )
{
    gint32 ret = 0;
    do{
    ...
    fuse_file_info * fi )
{
    ...
    return pObj->fs_releasedir( path, fi );
}
