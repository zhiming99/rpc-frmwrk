 ...
 * 如果想快速上手，没有耐心搞明白各种证书和密钥的关系，也可以使用`rpcfg.py`的[安全标签页](../../pics/rpcfg2.png)上的`Gen Self-Signed Keys`生成一组自签名的密钥。然后用`Installer`按钮生成分别用于服务器端和客户端的自解压安装包。在待部署的机器上运行安装包，部署密钥和配置系统。此`安装包`可以部署rpc-frmwrk的运行参数，密钥，web服务器的配置，kerberos服务器的配置，以及已经build好的deb包或rpm包，由`ridlc`生成的或者用户编写的客户端或者服务器端可执行程序需自行拷贝到目标机器上。
    1) 安装包的名字如下图所示   
