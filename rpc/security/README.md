...
##### 2. Setup the client machines. Client machines are those devices via which the users can access the the RPC services.
  ...
  * Put the same `krb5.conf` as the one on the `kdc server` to the directory `/etc`. (Note: if the client is of a different linux distribution, you may need to tweak the `krb5.conf` which can be quite different from the one on the `kdc server`)
