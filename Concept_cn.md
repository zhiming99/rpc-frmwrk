...
## DBus的作用
DBus是一个本地的消息总线，似乎和RPC关系不大。实际上，`rpc-frmwrk`的服务器预想的运行环境下，不仅会接受来自远程互联网的请求，也会接收到同一台机器上的其他服务器进程的请求，以及转发一些请求到其他的服务器进程。虽然这些服务器也可以使用TCP进行传输，但是比起DBus这类轻量级的IPC传输，就显得笨拙低效了。因此DBus就充当了本地间的通信的通道。服务器端，DBus是必须的组件，而客户端则可以运行在没有DBus的环境中。
...
## 监控
* `rpc-frmwrk`的监视器子系统可以提供系统的实时运行状态，帮助用户合理配置资源和调节运行参数。监视器系统也可配置调度和点位日志功能，通过长期记录监控点位数据，调优系统匹配用户的运行环境。

## 注册表
* `rpc-frmwrk`的监视器的关键部件，用于存储应用信息，用户信息和运行态参数，功能类似windows的注册表。详细信息可参考注册表的[介绍](./monitor/registry/README_cn.md)。
## 性能
`rpc-frmwrk`在一个i7的4核笔记本上的测试中，1000-5000个连接测试中，2000个连接时达到吞吐量峰值，单个`Request`的平均响应时间在1.1ms左右。大流量并发时服务器的吞吐量大概在每秒2500 Requests. 

## `ridl`接口描述语言与RPC程序的开发
