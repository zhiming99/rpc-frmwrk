...
// Include stlexer.h first to get YYLTYPE2 definition
// This must come before any other includes that use YYLTYPE2
// #include "stlexer.h"
...
#define CONFLICT_STATE      616: case 656
// the state to replace TOK_MINUS with TOK_SUB
#define SUBEXPR_STATE     69
#define LVALUE_BIT_STATES      63: case 119: case 315: case 441
#define METHOD_RET_STATE     661
#define USING_SEP_STATE     100
#define CASESEL_CHECK_STATES     std::vector<int>({ 74, 75 })
...
struct CTokenBuffer
{
    ...
    inline const CTokenInfo& GetCurTokBack(
        gint32 idx = 0 ) const
    {
        if( idx < 0 || m_queFuture.size() <= idx )
            return m_oEmpty;
        return *( m_queFuture.rbegin() + idx );
    }
...
struct CSTParserContext
{
    ...
    void* m_yypstate = nullptr;
    stdstr m_strCurFile;
    std::vector< stdstr > m_vecFiles;
    gint32 m_iLastCaseChk = 0;
    CTokenBuffer m_oTokens;

    // flag to use queued token instead tokens from
    // lexer
    bool m_bUseQueuedToken = false;
    // flag to show the lexer has reached the end of
    // the source file
    bool m_bEOF = false;
    ...
    void SetParser( void* pstate )
    { m_yypstate = pstate; }

    void* GetParser() const
    { return m_yypstate; }
    ...
    inline void SetEOF( bool bEOF )
    { m_bEOF = bEOF; }

    inline bool UseQueuedToken() const
    { return m_bUseQueuedToken; }

    inline void SetUseQueuedToken( bool bUse )
    { m_bUseQueuedToken = bUse; }
    ...
    inline const CTokenInfo& GetCurTokBack(
        gint32 idx = 0 ) const
    { return m_oTokens.GetCurTokBack( idx ); }
