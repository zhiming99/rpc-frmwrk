...
const { randomInt, ERROR, Int32Value, USER_METHOD } = require( '../../../../js/combase/defines' );
...
class CAppMonitor_CliImpl extends CAppMonitorclibase
{
    ...
    // IAppStoreEx::GetPtLogInfoCallback
    // request callback
    GetPtLogInfoCallback( oContext, ret,
        mapPtLogInfo // [String,ObjPtr]
    )
    {
        // add code here
        // 'oContext' is the parameter passed to this.GetPtLogInfo
        // 'ret' is the status code of the request
        // if ret == errno.STATUS_SUCCESS, the following parameters are valid response from the server
        // if ret < 0, the following parameters are undefined
    }

    // IAppStoreEx::GetPtLogCallback
    // request callback
    GetPtLogCallback( oContext, ret )
    {
        // add code here
        // 'oContext' is the parameter passed to this.GetPtLog
        // 'ret' is the status code of the request
        // if ret == errno.STATUS_SUCCESS, the following parameters are valid response from the server
        // if ret < 0, the following parameters are undefined
    }
    ...
    */
    OnDataReceivedImpl( hStream, oBuf )
    {
        ...
        if( globalThis.oStmCtx && globalThis.oStmCtx.OnDataReceived)
            return globalThis.oStmCtx.OnDataReceived( hStream, oBuf );
        return -14;
    }
    ...
    */
    OnStmClosedImpl( hStream )
    {
        ...
        if( globalThis.oStmCtx && globalThis.oStmCtx.OnStmClosed)
            globalThis.oStmCtx.OnStmClosed( hStream );
    }
}
