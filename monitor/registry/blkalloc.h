...
#define BLOCK_IDX_MASK                      ( BLOCKS_PER_GROUP - 1 )

#define GROUP_SHIFT                         ( find_shift( PAGE_SIZE ) )
...
#define GROUP_IDX_MASK                      ( BLKGRP_NUMBER - 1 )

// group index in a block index
#define GROUP_IDX_SHIFT                     ( 1 << ( find_shift( GROUP_SIZE ) ) )

#define LA_ADDR( _byte_addr )               ( _byte_addr - SUPER_BLOCK_SIZE - GRPBMP_SIZE )
#define ABS_ADDR( _la_addr )                ( _la_addr + SUPER_BLOCK_SIZE + GRPBMP_SIZE )
...
struct CBlockAllocator
{
    ...
    guint32     m_dwGrpNum = BLKGRP_NUMBER;
    gint32      m_iFd;
    ...
    // write pBuf to qwLogicAddr, till the end of the
    // block or the end of the pBuf
    gint32 WriteBlock(
        guint64 qwLogicAddr, BufPtr pBuf );

    // write pBuf to an array of blocks, till all the
    // blocks are written or all the bytes are written
    // from the pBuf.
    gint32 WriteBlocks( const guint64* pBlocks,
        guint32 dwNumBlocks, BufPtr pBuf );

    // read block at qwLogicAddr to pBuf, till the end of the
    // block.
    gint32 ReadBlock(
        guint64 qwLogicAddr, BufPtr pBuf );

    // read blocks to pBuf specified by logic addresses
    // from pBlocks, till the end of the pBlocks.
    gint32 ReadBlocks( const guint64* pBlocks,
        guint32 dwNumBlocks, BufPtr& pBuf );
};
