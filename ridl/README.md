...
* **maincli.cpp, mainsvr.cpp**: as the name indicate, the two files define main function of the proxy and server respectively. Each file contains a same-name function, that is `maincli` for maincli.cpp and `mainsvr` for mainsvr.cpp, which is the ideal place to add your business code if you are still new to `rpc-frmwrk`.
