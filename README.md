...
#### Monitoring
`rpc-frmwrk` provides a web-based monitoring program, which allows you to easily monitor `rpc-frmwrk` servers and configure runtime parameters from various platforms. The monitoring program features access control, so different users can access different content. Access control can be easily customized from the backend. The monitoring system can enable logging for specific setpoints at the backend, providing more detailed historical data for analysis and optimization. For information on setting up the monitoring functionality, please refer to [this article](./monitor/client/js/appmoncli/README.md).
