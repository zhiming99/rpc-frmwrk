...
2. 在运行之前，你需要配置`rpc-frwmrk`的守护进程。`rpcfg`是`rpc-frmwrk`的自带配置工具，它是配置`rpc-frmwrk`的强大工具，可以节省大量查找和学习时间，甚至包括一个简单的SSL密钥管理器。关于配置工具`rpcfg`，可以参考它的[README](https://github.com/zhiming99/rpc-frmwrk/tree/master/tools/README.md).
...
## 开发
通过`rpc-frmwrk`的接口描述语言(`ridl`)和编译器(`ridlc`), `rpc-frmwrk`提供三种语言(C++, Python, Java)的三种模式的`RPC`应用程序的开发.关于`ridl`详细信息请参考[readme](https://github.com/zhiming99/rpc-frmwrk/blob/master/ridl/README_cn.md)
