...
4. 待一切就绪后，分别启动用户服务器和客户端。

### 部署
* 多数情况下，开发主机和目标主机是不同的主机，针对这种情况，`rpcfg.py`和`rpcfctl`可以生成目标机器的`安装包`。`安装包`安装`rpc-frwmwrk`的运行库并执行系统一级的设置。设置内容包括`rpc-frmwrk`的运行参数，`Apache`或者`Nginx`web服务器的设置，SSL密钥分发，以及认证所需的必要信息。详细说明请参考`rpcfg.py`的[使用说明](./tools/README_cn.md)有关安装包的部分. 
* 在没有图形界面的环境下，可以使用`rpcfctl`可以完成相关的配置任务。有关`rpcfctl`的详细信息请参考这篇[文章](./monitor/appmon/rpcfctl_cn.md)。
